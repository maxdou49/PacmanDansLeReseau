/*
 * L'affichage du terrain du jeu Pacman
 */
...
public class PanelPacmanGame extends JPanel {
	...
	//L'etat appeuré est indépendant pour chaque fantome
	private ArrayList<Boolean> ghostsScarred;
	...
	public PanelPacmanGame(Maze maze) {
		...
		ghostsScarred = new ArrayList<Boolean>(this.m.getGhosts_start().size());
	}
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < ghosts_pos.size(); i++) {
			...
			if (ghostsScarred.size() > i && ghostsScarred.get(i)) {
	...
	public void setGhostsScarred(ArrayList<Boolean> ghostsScarred) {
